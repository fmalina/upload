...
urlpatterns = [
    # XHR file handler to upload files
    url(r'^$', upload, name='upload'),
    # XHR handler for existing collection
    url(r'^(?P<pk>[0-9]+)$', upload, name='upload_edit'),

    # Post collection
    url(r'^post$', FilesEditView.as_view(), name="upload_col"),
    # Edit collection
    url(r'^(?P<pk>[0-9]+)/edit$', FilesEditView.as_view(), name='upload_col_edit'),
